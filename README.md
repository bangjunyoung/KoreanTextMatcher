...
- 초성 매칭이 더 정교한 음절 근사 매칭으로 대체되었다.
  예를 들어 3.0에서는 '빨래'라는 텍스트에 '빨래'라는 완전 일치어 또는 'ㅃㄹ'이라는 초성 패턴만이 매칭되었다면, 4.0에서는 '빨래', '빠래', 'ㅃㄹ', 'ㅂㄹ' 등 음절 앞부분의 일부 자소만으로도 매칭이 가능해졌다.
- 중성/종성 관련 API와 음절을 자모로 분해하는 API가 추가되었다.
- 소스/바이너리 호환성 문제 때문에 JDK 11만을 타겟으로 하도록 바꾸었다.
